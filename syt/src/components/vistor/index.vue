<template>
    <div class="vistor">
        <div class="top">
            <div class="left">
                <span class="insurance">{{ user.isInsure == 1 ? '医保' : '自费' }}</span>
                <span class="username">{{ user.name }}</span>
            </div>
            <div class="right">
                <el-button circle type="primary" :icon="Edit" @click="handler"></el-button>
                <el-popconfirm :confirm="removeUser" title="`确定删除${user.name}?`">
                    <template #reference>
                        <el-button v-if="route.path == '/user/patient'" circle type="danger" :icon="Delete"></el-button>
                    </template>
                </el-popconfirm>
            </div>
        </div>
...
let emits = defineEmits(['changeScene','deleteUser'])
// console.log(props.user);
console.log('indexindex', props.currentIndex, props.index);
let route = useRoute()
let router = useRouter()
function handler() {
    if (route.path == '/user/patient') {
        emits('changeScene', props.user)
    } else {
